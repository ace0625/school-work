...
//student node default constructor
snode::snode()
{
	next = NULL;
}

//student node default denstructor
snode::~snode()
{}

//student node copy constructor
...
{}
//student node go to the next node
snode *& snode::go_next()
{
	return next;
}
//to see if there is next node
bool snode::if_next()
{
	if(next) 
		return true;
...
//display the each node data
void snode::display_snode()
{
	display_sclass();
}
//retrieve the data by class type and class code number
...
{
	if(term == a_term) //if it's the same return true 
		return true;
