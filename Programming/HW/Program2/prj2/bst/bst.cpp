...
// add bst node
void bst::add(bst_node*& root, const int a_term, sclass* a_sclass)
{
	if(!root) {
		root = new bst_node;
		...
		add(root->go_left(), a_term, a_sclass);
	else
		add(root->go_right(), a_term, a_sclass);
}
...
int main()
{
	...
	cout << "\n\n** add **************************" << endl;
	//char* a_course_type, int a_course_code, int a_course_term, char a_course_grade
	// temp data
	char a_course_type [] = "CS";
	int a_course_code = 163;
	int a_course_term = 20151;
	char a_course_grade = 'A';
	
	sclass* s1 = new sclass(a_course_type, a_course_code, a_course_term, a_course_grade);
	b.add(a_course_term, s1);

	a_course_term = 20152;
	sclass* s2 = new sclass(a_course_type, 163, a_course_term, a_course_grade);
	b.add(a_course_term, s2);

	sclass* s3 = new sclass(a_course_type, 202, a_course_term, a_course_grade);
	b.add(a_course_term, s3);

	sclass* s4 = new sclass(a_course_type, 121, a_course_term, a_course_grade);
	b.add(a_course_term, s4);
