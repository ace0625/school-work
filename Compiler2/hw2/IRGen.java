...
public class IRGen {
  ...
  private static ClassInfo createClassInfo(Ast.ClassDecl n) throws Exception 
  {
      ...
      int offset_val = cinfo.objSize;
      for (Ast.VarDecl v : n.flds)
      {
        cinfo.offsets.put(v.nm, offset_val);
        offset_val += gen(v.t).size;
      }

      cinfo.objSize = offset_val;
      return cinfo;
    //  ... NEED CODE ...
  ...
  static IR.Func gen(Ast.MethodDecl n, ClassInfo cinfo) throws Exception 
  {
      ...
      if(!n.nm.equals("main"))
      {
          gl = new IR.Global("_" + cinfo.name + "-" + n.nm);
          params.add(new IR.Id("obj"));
      }
      else
          gl = new IR.Global("_main");
      ...
      for(Ast.VarDecl v : n.vars)
      {
          ...
          List<IR.Inst> init = gen(v, cinfo, env);
          if(init != null)
          {
      ...
      return new IR.Func(gl, params, vars, code);
    //  ... NEED CODE ...
  ...
  static List<IR.Inst> gen(Ast.VarDecl n, ClassInfo cinfo, 
    ...
    if(n.init != null)
    {
        CodePack codeP = gen(n.init, cinfo, env);
        IR.Move m = new IR.Move(new IR.Id(n.nm), codeP.src);
        code.addAll(codeP.code);
        code.add(m);
        return code;
    }
    else
  ...
  static List<IR.Inst> gen(Ast.Assign n, ClassInfo cinfo, Env env) throws Exception {
      ...
      CodePack cr = gen(n.rhs, cinfo, env);
      code.addAll(cr.code);
      ...
      if(n.lhs instanceof Ast.Id)
      {
          ...
          {
              IR.Dest l = new IR.Id(((Ast.Id) n.lhs).nm);  
              code.add(new IR.Move(l, cr.src));
              code.addAll(cr.code);
          }
          else
          {
              Ast.Field fl = new Ast.Field(Ast.This, ((Ast.Id) n.lhs).nm);
              CodePack codeP = gen(fl.obj, cinfo, env);
              ClassInfo cls = getClassInfo(fl.obj, cinfo, env);
              int ofs = cls.fieldOffset(fl.nm);
              IR.Addr addr = new IR.Addr(codeP.src, ofs);
              Ast.Type tp = cls.fieldType(fl.nm);
              IR.Store store = new IR.Store(gen(tp), addr, cr.src);
              code.addAll(codeP.code);
              code.add(store);
          }
      ...
      {
          CodePack codeP = gen(((Ast.Field) n.lhs).obj, cinfo, env);
          ClassInfo cls = getClassInfo(((Ast.Field) n.lhs).obj, cinfo, env);
          int ofs = cls.fieldOffset(((Ast.Field) n.lhs).nm);
          IR.Addr addr = new IR.Addr(codeP.src, ofs);
          Ast.Type tp = cls.fieldType(((Ast.Field) n.lhs).nm);
          IR.Store st = new IR.Store(gen(tp), addr, cr.src);
          code.addAll(codeP.code);
          code.add(st);
      }
			  ...
			  ClassInfo cinfo, Env env, boolean retFlag) throws Exception 
  {
    ...
    List<IR.Inst> code = new ArrayList<IR.Inst>();
    List<IR.Src> arg = new ArrayList<IR.Src>();

    ClassInfo clinfo = getClassInfo(obj, cinfo, env);

    ClassInfo binfo = clinfo.methodBaseClass(name);

    IR.Global globNmae = new IR.Global("_" + binfo.name + "_" + name);

    CodePack codeP = gen(obj, cinfo, env);
    arg.add(codeP.src);

    for(Ast.Exp e : args)
    {
       CodePack codeP2 = gen(e, cinfo, env);
       code.addAll(codeP2.code);
       arg.add(codeP2.src);
    }
    IR.Temp tm = null;
    Ast.Type type = null;
    ...
    {
        tm = new IR.Temp();
        type = clinfo.methodType(name);
    }

    code.add(new IR.Call(globNmae, false, arg, tm));

    return new CodePack(gen(type), tm, code);
