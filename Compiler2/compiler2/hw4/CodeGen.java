...
class CodeGen {
  ...
  static void gen(IR1.Func n) throws Exception { 
	...
	src = new IR1.Id(p);
	if (!X86.argRegs[k].equals(regMap.get(src)))
          if (regMap.containsKey(src))
            X86.emit2("movq", X86.argRegs[k], regMap.get(src));
	++k;
      }
    } else{
  ...
  static void gen(IR1.Binop n) throws Exception {
    if (regMap.containsKey(n.dst)) {
  	...
  	X86.emit1("idivq", rr);
        ...
        if (!rr.equals(regMap.get(n.dst))){
	  if (lr.equals(regMap.get(n.dst))){
	    //do nothing
 	  } else{
            X86.emit2("movq", lr, dstr);  
	  }
	  ...
	  X86.emit2("orq", rr, dstr);
        ...
        X86.Reg br = X86.resize_reg(X86.Size.B, dstr);
  ...
  static void gen(IR1.Move n) throws Exception {
    if (regMap.containsKey(n.dst)){
      if (n.src instanceof IR1.IntLit || n.src instanceof IR1.StrLit
      				      || n.src instanceof IR1.BoolLit){
        gen_source(n.src, regMap.get(n.dst));
      }else{
        X86.Reg sr = gen_source(n.src, tempReg1);
        if (! sr.equals(regMap.get(n.dst)))
          X86.emit2("movq", sr, regMap.get(n.dst));
      }
    } else{
      //dead
...
System.out.println("not implement yet");
  ...
  static void gen(IR1.Jump n) throws Exception {
    X86.emit1("jmp", new X86.GLabel(fnName+"_"+n.lab.name));
  }
  ...
  static void gen(IR1.Return n) throws Exception {
    if (n.val != null){
      if (n.val instanceof IR1.IntLit || n.val instanceof IR1.StrLit
                                      || n.val instanceof IR1.BoolLit){
        gen_source(n.val, X86.RAX);
      }else{
        X86.Reg vr = gen_source(n.val, tempReg1);
	if (! vr.equals(X86.RAX))
	  X86.emit2("movq", vr, X86.RAX);
      }
    }
