...
public class Checker {
  ...
  // verifyReturns function that runs at end of each method to verify return statements
  private static boolean verifyReturns()
  {
	  boolean success = false; //success
	  boolean ifrtn = false;  //return
	  boolean nested = false; // nesting
	  boolean nestrtn = false; // both nested have returns
	  for(Pair p : returns)
	  {
 ...
 //
  static void check(Ast.MethodDecl n) throws Exception {
	...
	if(n.t != null)
	{
		...
		if(!success)
		{
			throw new TypeException("From MethodDecl, return statement is missing");
		}
	}
  } 
  ...
  static void check(Ast.Param n) throws Exception {
	if(n.t instanceof Ast.ObjType)
	{
		...
		if(!classEnv.containsKey(clsnm.nm))
			throw new TypeException("From Param, not able to find class " + clsnm.nm);
	}
  }
  ...
  static void check(Ast.VarDecl n) throws Exception {
	...
	if(n.init != null)
	{
		...
		if(ini instanceof Ast.ObjType)
		{
			...
			{
				throw new TypeException("From VarDecl, not able to find class " + ((Ast.ObjType)ini).nm);
			}
		}
		if(!assignable(n.t, ini))
		{
			throw new TypeException("From VarDecl, rhs cannot be initialized to lhs " + ((Ast.ObjType)ini).nm);
		}
	}
  }
  ...
  static void check(Ast.Assign n) throws Exception {
	...
	{
		throw new TypeException("From Assign, lhs and rhs types don't match: " + exp1 + " <- " + exp2); // verify this
	}
  }
  ...
  static void check(Ast.CallStmt n) throws Exception {
	...
	{
		throw new TypeException("From CallStmt, Object type1 " + n.obj);
	}
	if(n.obj instanceof Ast.Id)
	{
		...
		{
			throw new TypeException("From CallStmt, Class doesn't exist");
		}
		String cls = classObjects.get(clsnm.nm);
		Ast.MethodDecl mthd = classEnv.get(cls).findMethodDecl(n.nm);
		if(mthd == null)
		{
			throw new TypeException("From CallStmt, Method doesn't exist");
		}
		if(mthd.params.length != n.args.length)
		{
			throw new TypeException("From CallStmt, Param and arg counts are not matching: " + mthd.params.length + " vs. " + n.args.length);
		}
		for(int i = 0; i < mthd.params.length; ++i)
		{
			...
			{
				throw new TypeException("From CallStmt, Param and arg types are not matching: " + type1 + " vs. " + type2);
			}
			if(type1 instanceof Ast.ObjType)
			{ 
	...
	else
	{
		...
		if(var == null)
		{
			throw new TypeException("From CallStmt, not able to find method " + n.nm);
		}
		if(var.params.length != n.args.length)
		{
			throw new TypeException("From CallStmt, Param and arg counts are not matching: " + var.params.length + " vs. " + n.args.length);
		}
		for(int i = 0; i < var.params.length; ++i)
		{
			...
			if(!(type1.getClass().equals(type2.getClass())))
			{
				throw new TypeException("From CallStmt, Param and arg counts are not matching: " + type1 + " vs. " + type2);
			}
			if(type1 instanceof Ast.ObjType)
			{ 
				Ast.ObjType obj1 = (Ast.ObjType)type1;
  ...
  static void check(Ast.If n) throws Exception {
	...
	{
		throw new TypeException("From if, Cond exp type is not boolean: " + type);
	}
  ...
  static void check(Ast.While n) throws Exception {
	...
	if(!(type instanceof Ast.BoolType))
	{
		throw new TypeException("From While, Cond exp type is not boolean: " + type);
	}
  ...
  static void check(Ast.Print n) throws Exception {
	...
	{
		throw new TypeException("From Print, arg type isn't int, boolean, or string: " + type);
	}
  }
  ...
  static void check(Ast.Return n) throws Exception {
	...
	{
		throw new TypeException("From Return,return value of type is missing " + thisMDecl.t);
	}
	else if((n.val != null && thisMDecl.t == null))
	{
		throw new TypeException("From Return, Unexpected return value");
	}
	if(n.val == null && thisMDecl.t == null)
	{
		return;
	...
	{
		throw new TypeException("From Return, Return type mismatch: " + thisMDecl.t + " <- " + rtn);
	}
  }
	...
	if (comparable(e1,e2))
	{
		switch(n.op)
		{
			...
			case DIV:
				...
				throw new TypeException("From Binop, inappropriate operand types for binary operator: " + n.op);
			case AND:
			case OR:
				if(e1 instanceof Ast.IntType)
					throw new TypeException("From Binop, inappropriate operand types for binary operator: " + n.op);
		...
		throw new TypeException("From Binop, Operand types aren't matching: " + e1 + " " + n.op + " " + e2);
	}
  }
  ...
  static Ast.Type check(Ast.Unop n) throws Exception {
	if(!(n.e instanceof Ast.IntLit || n.e instanceof Ast.BoolLit))
	{
		...
		if(type instanceof Ast.IntType)
		{
			...
			{
				throw new TypeException("From Unop, inappropriate operand type: " + n.op + " " + type);
			}
		}
		else if(type instanceof Ast.BoolType)
		{
			...
			{
				throw new TypeException("From Unop, inappropriate operand type: " + n.op + " BoolType" );
			}
		}
	}
	else if(n.e instanceof Ast.IntLit)
	{
		...
		{
			throw new TypeException("From Unop, inappropriate operand type: " + n.op + " IntType");
		}
	}
	...
	throw new TypeException("From Unop, Type is not integer or boolean"); // should never reach this
  }
  ...
  static Ast.Type check(Ast.Call n) throws Exception {
	if(!((n.obj instanceof Ast.Id) || (n.obj instanceof Ast.This)))
	{
		throw new TypeException("From Call, not an object type: " + n.obj);
	}
	if(n.obj instanceof Ast.Id)
	{
		Ast.Id clsnm = (Ast.Id)n.obj;
		if(!(classObjects.containsKey(clsnm.nm)))
		{
			throw new TypeException("From call, Class doesn't exist: " + clsnm.nm);
		}
		String cls = classObjects.get(clsnm.nm);
		Ast.MethodDecl mthd = classEnv.get(cls).findMethodDecl(n.nm);
		if(mthd == null)
		{
			throw new TypeException("From Call, Method doesn't exist: " + n.nm);
		}
		if(mthd.params.length != n.args.length)
		{
			throw new TypeException("From Call, Param and arg counts aren't matching: " + mthd.params.length + " vs. " + n.args.length);
		}
		for(int i = 0; i < mthd.params.length; ++i)
		{
			Ast.Type type1 = mthd.params[i].t;
			...
			if(!(type1.getClass().equals(type2.getClass())))
			{
				throw new TypeException("From Call, Param and arg types aren't matching: " + type1 + " vs. " + type2);
			}
			if(type1 instanceof Ast.ObjType)
			{ 
				Ast.ObjType obj1 = (Ast.ObjType)type1;
				...
				if(classEnv.containsKey(obj1.nm) && classEnv.containsKey(obj2.nm))
				{
					if(!assignable(obj1,obj2))
					{
						throw new TypeException("From Call, Param and arg types are not matching: " + type1 + " vs. " + type2);
					}
				}
				else
				{
					throw new TypeException("From Call, Method doesn't exist:");
				}
			}
		}
		return mthd.t;
	...
	else
	{
		...
		if(var == null)
		{
			throw new TypeException("From Call, not able to find method " + n.nm);
		}
		if(var.params.length != n.args.length)
		{
			throw new TypeException("From Call, Param and arg types are not matching:" + var.params.length + " vs. " + n.args.length);
		}
		for(int i = 0; i < var.params.length; ++i)
		{
			Ast.Type type1 = var.params[i].t;
			...
			{ 
				...
				{
					throw new TypeException("From Call, Class doesn't exist.");
				}
			}
		}
		return var.t;
  ...
  static Ast.Type check(Ast.NewArray n) throws Exception {
	if(n.et instanceof Ast.IntType || n.et instanceof Ast.BoolType)
	{
		...
		{
			throw new TypeException("From NewArray, Index can't be negative");
		}
	}
	else
	{
		throw new TypeException("From NewArray, Type is not integer or boolean");
	}
  }
  ...
  static Ast.Type check(Ast.ArrayElm n) throws Exception {
	...
	{
		throw new TypeException("From ArrayElm, it should be Id exp");
	}
	...
	if(!(typeEnv.containsKey(objnm.nm)))
	{
		...
		if(var == null)
		{
			throw new TypeException("From ArrayElm, Array doesn't exist " + objnm.nm + " " + n.ar + " " + n.idx);
		}
		else
		{
			if(!(var.t instanceof Ast.ArrayType))
			{
				throw new TypeException("From ArrayElm, Array doesn't exist");
			}
			if(!(n.idx instanceof Ast.IntLit))
			{
				...
				{
					throw new TypeException("From ArrayElm,Index is not integer: " + check(n.idx));
				}
			}
			else
			{
	...
	else 
	{
		...
		{	
			throw new TypeException("From ArrayElm, Object is not array: " + typeEnv.get(objnm.nm));
		}
		if(!(n.idx instanceof Ast.IntLit))
		{
			Ast.Type index = check(n.idx);
			...
			{
				throw new TypeException("From ArrayElm, index isn't integer: " + check(n.idx));
			}
		}
		else
		{
  ...
  static Ast.Type check(Ast.NewObj n) throws Exception {
	...
	{
		throw new TypeException("From NewObj, not able to find class " + n.nm);
	}
  }
  ...
  static Ast.Type check(Ast.Field n) throws Exception {
	...
	{
		throw new TypeException("From Field, object is not ObjectType: " + n.obj);
	}
	if(n.obj instanceof Ast.Id)
	{
		Ast.Id clsnm = (Ast.Id)n.obj;
		...
		{
			throw new TypeException("From Field, object undeclared: " + clsnm.nm);
		}
		Ast.Type type = typeEnv.get(clsnm.nm);
		if(!(type instanceof Ast.ObjType))
		{
			throw new TypeException("FromField, object is not of ObjType: " + type);
		}
		if(!(classObjects.containsKey(clsnm.nm)))
		{
			throw new TypeException("From Field, object class doesn't exist: " + clsnm.nm);
		}
		...
		throw new TypeException("From Field, not able to find field " + n.nm);
	}
	else if(n.obj instanceof Ast.Field)
	{
		...
		{
			throw new TypeException("From Field, object is not of ObjType: " + next);
		}
		Ast.ObjType temp = (Ast.ObjType)next;
		if(!(classEnv.containsKey(temp.nm)))
		{
			throw new TypeException("From Field, object class doesn't exist: " + temp.nm);
		}
		if(classEnv.get(temp.nm).findFieldDecl(n.nm) != null)
		{
			return classEnv.get(temp.nm).findFieldDecl(n.nm).t;
		}
		throw new TypeException("From ield, not able to find field " + n.nm);
	}
	else
	{
		...
		if(var == null)
		{
			throw new TypeException("From Field, field not in class");
		}
		return var.t;
	}
  }
 ...
 //
  static Ast.Type check(Ast.Id n) throws Exception {
	...
	else
	{
		...
		if(temp == null){
			throw new TypeException("From Id, not able to find variable " + n.nm);
		}
		return temp.t;
	}
  }
